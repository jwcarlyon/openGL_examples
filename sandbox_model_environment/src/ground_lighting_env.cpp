...
/*Successfully compiled using command $ g++ ground_lighting_env.cpp glad.c -lGL -lglfw3 -Iglad -lX11 -ldl -lpthread -lassimp
  !Update, 19Jul2022 added cmake to manage project,
    !!!!Delete the build folder in the project root directory, then follow:
    mkdir build
    cd build
    cmake ../src
    cmake --build .
  on inteli9 10900k, asus hero xii rtx3060 graphics all air cooled
  */
...
#include "stb_image.h"
//Miscellaneous file related libraries
#include <filesystem>
...
// std::vector<float> collect_model_vertices_vector(Model model);
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Vehicle, pavement and sandbox", NULL, NULL);
    if(window == NULL)
    {
...
// std::vector<float> collect_model_vertices_vector(Model& model_ref){
//     std::vector<float> model_vertices_vector;
//         for(std::vector<Mesh>::iterator mesh_it = model_ref.meshes.begin(); mesh_it != model_ref.meshes.end(); ++mesh_it){
//             Mesh mesh = *mesh_it;
//             for(std::vector<Vertex>::iterator vertex_it = mesh.vertices.begin(); vertex_it != mesh.vertices.end(); ++vertex_it){
//                 Vertex vertex_struct = *vertex_it;
//                 model_vertices_vector.push_back(vertex_struct.Position[0]);
//                 model_vertices_vector.push_back(vertex_struct.Position[1]);
//                 model_vertices_vector.push_back(vertex_struct.Position[2]);
//             }
//         }
//     return model_vertices_vector;
// }
